...
def new_hits(ranks, k):
    top = len([rank for rank in ranks if rank != 0 and rank <= k])
    non_zeros = len([rank for rank in ranks if rank != 0])
...
def run_all(hs_value1, hs_value2, hs_value3, rank_files):
    ...
    for file in rank_files:
        ...
        num_zeros = sum(1 for element in all_ranks if element == 0)
        num_non_zeros = sum(1 for element in all_ranks if element != 0)
        ...
        hs1 = hits_at(all_ranks, hs_value1) # proportion of
        hs2 = hits_at(all_ranks, hs_value2)
        hs3 = hits_at(all_ranks, hs_value3)
    ...
    results.to_csv('Final_Evaluation_Results_1000.csv')
